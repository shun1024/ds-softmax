...
def noisy_top_k_gating(x,
    ...
    Args:
      ...
      complex_gating: complex gating network
    ...
    with tf.variable_scope(name, default_name="noisy_top_k_gating"):
        ...
        if complex_gating:
            w1_gate = tf.get_variable(
                "w1_gate", [input_size, input_size], tf.float32, initializer)

            hidden_logits = tf.nn.relu(tf.matmul(x, w1_gate))
            w2_gate = tf.get_variable(
                "w2_gate", [input_size, num_experts], tf.float32, initializer)
            clean_logits = tf.matmul(hidden_logits, w2_gate)
        else:
        ...
        if noisy_gating:
            w_noise = tf.get_variable("w_noise", [input_size, num_experts], tf.float32, initializer)
        ...
        # assign softmax before top-k to preserve information - Shun
...
def local_moe(x,
    ...
    Args:
      ...
      num_experts: an integer - number of experts
      k: an integer - how many experts to use for each batch element
      noise: noisy gating network
      loss_coef: a scalar - multiplier on load-balancing losses
      complex_gating: complex_gating gating network
